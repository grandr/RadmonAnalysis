...
TPython::LoadMacro("fluenceRatios.py");
FluenceRatios p;
p.fillHistos();
p.drawHisto("mfit")
...
##2015
#radmonLumiFilePattern = '/home/data/RadmonLumi/2015/radmonLumi__XXX__.root'
##Fills to use
#fills = [  4341, 4342, 4349, 4356, 4360, 4363, 4364, 4376, 4381, 4384, 4386, 4391, 4393, 4397, 4398, 4402, 4410, 4418, 4420, 4423, 4426, 4428, 4432, 4434, 4435, 4437, 4440, 4444, 4448, 4449, 4452, 4455, 4462, 4463, 4464, 4466, 4467,4476, 4477, 4479, 4485, 4495, 4496, 4499, 4505, 4509, 4510, 4511, 4513, 4518, 4519, 4522, 4525, 4528, 4530, 4532, 4536, 4538, 4540, 4545, 4555, 4557, 4562, 4565] 
         ##4560, 4569]
         ##3960, 3962, 3965, 3971, 3974,3976, 3981, 3983, 3986, 3988, 3992, 3996, 4001, 4006, 4008, 4019, 4020, 4201, 4205, 4207, 4208, 4210, 4211, 4212, 4214, 4219, 4220, 4224, 4225, 4231, 4243, 4246, 4249, 4254, 4256, 4257, 4266, 4268, 4269, 4322, 4323, 4332, 4337,
         
#fillReportFile = '../Config/FillReport_1446656923991.xls'
         
#2016
radmonLumiFilePattern = '/home/data/RadmonLumi/2016/radmonLumi__XXX__.root'
# Fills to use
fills = [ 4942, 4945, 4947, 4953, 4954, 4956, 4958, 4960, 4961, 4964, 4965, 4976, 4979, 4980, 4984, 4985, 4988, 4990, 5005, 5013, 5017, 5020, 5021, 5024, 5026, 5027, 5028, 5029, 5030, 5038, 5043, 5045, 5048, 5052,]
#4888, 4889, 4890, 4892, 4895, 4896, 4905, 4906, 4910, 4915, 4919, 4924, 4925, 4926, 4930, 4935, 4937]

fillReportFile = '../Config/FillReport.xls'

(xmin, xmax) = (0., 20.)
deltaT = 2400 # Warmup cut 
nBins = 500
xmax = 0.

minPeakInstLumi = 1000.
minBfield = 3.6
...
class FluenceRatios:
    ...
    def fillHistos(self):
        ...
        for fill in fills:
            ...
            inputFile = radmonLumiFilePattern.replace('__XXX__', str(fill))
            try:
            ...
            for i in range(0, t.GetEntries()) :
                ...
                tsColl = -100000  # Start of collisions
                if tsColl < 0:
                    if t.rates[5] > rateColl and t.rates[5] < rateOn and  t.rates[12] > rateColl and t.rates[12] < rateOn: 
                        tsColl = t.tstamp  
        
                if t.tstamp > (tsColl + deltaT):    continue          
                if t.tstamp > tsEnd:    continue
            
                for key in self.indx.keys():
                    fluence = t.rates[self.indx[key]] * self.calib[key]
                    lumi = t.bestLumi
                
                    if fluence > rateOn or lumi <= 0.:
                        continue
                
                    self.histos[key].Fill(fluence/lumi)
